      ...
      <section className='max-w-7xl mx-auto'>
        ...
        <form action="submit" className='mt-16 max-w-3xl' onSubmit={HandleSubmit}>
          ...
          <div className='mt-5 flex gap-5'>
              <button disabled={(generatingImage || loading) ? true : false} type='button' onClick={HandleGenerate} className={`${(generatingImage || loading) ? 'cursor-not-allowed bg-green-300' : 'bg-green-700'} text-white font-medium rounded-md text-sm w-full xs:w-auto px-5 py-2.5 text-center`}>
          ...
          <div className='mt-10'>
            <p className='mt-2 text-[#666e75] dark:text-[#c2c4c7] text-[14px]'>Your image will be saved to your gallery when you share it with the community.</p>
              <button disabled={(generatingImage || loading) ? true : false} type='submit' className={`${(generatingImage || loading) ? 'cursor-not-allowed bg-[#8361d1]' : 'bg-[#6f45d1]'} mt-3 text-white font-medium rounded-md text-sm w-full xs:w-auto px-5 py-2.5 text-center`}>
