...
const App = () => {
  ...
  const HandleLogout = async (e) =>
  {
    ...
    try
    {
      const res = await axios.post('api/v1/auth/logout', null, { withCredentials: true })
      setOpen(false)
      dispatch(logout())
    }
    catch(err)
  ...
  useEffect(() => {
    ...
    let addClasses = ['ad', 'ads', 'adsbox', 'ad-placement', 'doubleclick', 'ad-placeholder', 'ad-badge'];
    detect.classList.add(...addClasses);
      ...
      <div id='detect' ref={detectRef}></div>
        <div className={`${isDark ? 'dark' : ''}`}>
          <header className='bg-[#fff] dark:bg-[#202124] w-full flex justify-between items-center bg-white sm:px-8 px-4 py-4 border-b border-b-[#e6ebff4] dark:border-b-[#3C4043]' onClick={HandleToggle}>
            <Link to='/'>
              {isDark ? (<img src={logoDark} alt="Dreamscape logo" className='w-[3.5em] sm:w-[75px] object-contain'/>) : (<img src={logoLight} alt="Dreamscape logo" className='w-[3.5em] sm:w-[75px] object-contain'/>)}
            </Link>
            <div className='flex items-center'>
              {isDark ? (<button onClick={() => setIsDark(!isDark)} title="Switch to light mode" className='hidden sm:block relative w-10 h-10 focus:outline-none focus:shadow-outline text-gray-500 hover:text-gray-800 dark:hover:text-gray-200 mr-[0.5vw]'>
              ...
              (<button onClick={() => setIsDark(!isDark)} title="Switch to dark mode" className='hidden sm:block relative w-10 h-10 focus:outline-none focus:shadow-outline text-gray-500 hover:text-gray-800 dark:hover:text-gray-200 mr-[0.5vw]'>
              ...
              {currentUser && !isSignInPage ? (
                <Link
                  to='/create'
                  className='hidden sm:block font-inter font-medium bg-[#6f45d1] text-white px-4 py-2 rounded-md'
                >
                  Create
                </Link>
              ) : !isSignInPage && (
              ...
              <div className='px-2 sm:hidden text-[#222328] dark:text-[#eeeeee] font-bold cursor-pointer'>
              ...
              <div ref={picRef} className='hidden sm:block px-4'>
                {currentUser && (currentUser.image ?
                  (<img src={currentUser.image} alt="User profile picture" className='w-10 h-10 rounded-full cursor-pointer' id='profile-pic' onClick={HandleToggle}/>) :
                  ...
                  <li className='profile_details flex justify-between mb-2 pb-4 px-3.5 border-b border-b-[#e6ebff4] dark:border-b-[#4b4f52]'>
                    <div className='flex items-center whitespace-nowrap overflow-hidden '>
                      {currentUser && (currentUser.image ? (currentUser.fromGoogle ?
                        (<div>
                          <img src={currentUser.image} alt="User profile picture" className='w-10 h-10 rounded-full'/>
                        </div>) :
              ...
              {(!currentUser && open) &&
              (<ul ref={userMenuRef} id='user-menu' className='user_menu absolute z-10 w-60 py-[0rem] bg-[#fff] dark:bg-[#2a2b2e] shadow-user_menu text-[#222328] dark:text-[#ffffff] top-[5rem] sm:top-[6rem] right-[1rem] sm:right-[1.5rem] rounded-xl'>
                  <li onClick={() => setOpen(false)}><Link to='/signin' className='rounded-t-xl flex sm:hidden justify-between items-center py-4 px-3.5 bg-[#6f45d1] text-[#fff]'><div className='flex items-center'><span className='mr-[1rem]'><LoginIcon/></span><span>Sign in</span></div></Link></li>
                  <li onClick={() => {setIsDark(!isDark); setOpen(false)}} className='flex sm:hidden justify-between items-center py-4 px-3.5 hover:bg-[#f2f2f2] dark:hover:bg-[#424346] cursor-pointer'>
                    <div className='flex items-center'>
                      {isDark ?
                        (<button title="Switch to light mode" className="focus:outline-none focus:shadow-outline mr-[1rem]">
                          <svg xmlns="http://www.w3.org/2000/svg" className="icon icon-tabler icon-tabler-sun" width="24" height="25.65" viewBox="0 0 24 24" strokeWidth="2" stroke="currentColor" fill="none" strokeLinecap="round" strokeLinejoin="round">
          ...
          <main className='relative sm:p-8 sm:pb-[6rem] px-4 py-8 pb-[3rem] w-full bg-[#fff] dark:bg-[#202124] min-h-[calc(100vh-73px)]' onClick={HandleToggle}>
            <Routes>
              <Route path='/adblock' element={getProperty === 'none' ? <Adblock/> : <Navigate to ='/' />}/>
              <Route path='/signin' element={!currentUser ? <Signin/> : <Navigate to='/'/>}/>
              <Route path='/' element={getProperty !== 'none' ? (currentUser ? <Home type=''/> : <Navigate to='/signin'/>) : (<Navigate to ='/adblock' />)}/>
              <Route path='/create' element={getProperty !== 'none' ? (currentUser ? <CreatePost/> : <Navigate to='/signin'/>) : (<Navigate to ='/adblock' />)}/>
              <Route path='/favourites' element={getProperty !== 'none' ? (currentUser ? <Home type={'favourites'}/> : <Navigate to='/signin'/>) : (<Navigate to ='/adblock' />)}/>
              <Route path='/gallery' element={getProperty !== 'none' ? (currentUser ? <Home type={'user'}/> : <Navigate to='/signin'/>) : (<Navigate to ='/adblock' />)}/>
              <Route path='/post/:id' element={getProperty !== 'none' ? <Post/> : (<Navigate to ='/adblock' />)}/>
              <Route path='/change-profile-pic' element={getProperty !== 'none' ? (currentUser ? (!currentUser.fromGoogle ? <ChangeProfilePic/> : <Navigate to='/'/>)  : <Navigate to='/signin'/>) : (<Navigate to ='/adblock' />)}/>
              <Route path='/users/:id/verify/:token' element={<EmailVerification/>}/>
              <Route path='/*' element={getProperty !== 'none' ? (<ErrorPage/>) : (<Navigate to ='/adblock' />)}/>
            </Routes>
          </main>
